        ...
        <div style={{ flex: 2 }}>
          ...
          <OrderTable>
            ...
            {productList.map(product => {
              const isSelected = selectedItems.some(i => i.productId === product.productId);
              return (
                  <tr key={product.productId} style={{ backgroundColor: isSelected ? '#f5fffa' : 'transparent' }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <Btn onClick={async () => {
            try {
              ...
              const msg = typeof err === 'string' ? err : err?.message || '처리 중 오류 발생';
              alert(msg);
            }
          }}>
            {isEdit ? '수정 완료' : '발주 등록'}
